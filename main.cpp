...
void svm(cv::Mat& trainingData, cv::Mat& trainingClasses, cv::Mat& testData, cv::Mat& testClasses) {
    ...
    if(plotSupportVectors) {
        ...
        for(int vecNum = 0; vecNum < svec_count; vecNum++) {
            ...
            cv::circle(plot_sv, Point(vec[0]*size, vec[1]*size), 3 , CV_RGB(0, 255, 0));
        }
...
int main() {
    //explain cv::Mat trainingData(numTrainingPoints, 2, CV_32FC1); numTrainingPoints=row, 2=column,CV_32FC1=type of matrix
    //declare data matrix for the training and testing
    ...
    //0=lower value, 1= upper value
    cv::randu(trainingData,0,1);
    cv::randu(testData,0,1);
    //below data is labeled for with function labelData(trainingData, eq). trainingData is the mat previously filled with 0's and 1's
    //eq=0 i.e sin(x*10) is evaluated
    //and store in trainingClasses matrix 
    cv::Mat trainingClasses = labelData(trainingData, eq);
    cv::Mat testClasses = labelData(testData, eq);
    //plot the trainingData and testData in white space with window name "Training Data" and "Test Data" respectly.
    //
